...
// Corporate order card payment checkout
router.post('/corporate/:corporateOrderId/card/checkout', asyncHandler(async (req: Request, res: Response) => {
  try {
    const corporateOrderId = parseInt(req.params.corporateOrderId);
    if (isNaN(corporateOrderId)) {
      return res.status(400).json({ error: 'Invalid corporate order ID' });
    }

    // Get corporate order with details
    const { storage } = await import('./storage.js');
    const order = await storage.getCorporateOrderWithDetails(corporateOrderId);
    if (!order) {
      return res.status(404).json({ error: 'Corporate order not found' });
    }
    ...
    const amountInCents = Math.round(parseFloat(order.totalInvoiceAmount) * 100);

    // Create checkout request
    const checkoutRequest = {
      amount: amountInCents,
      currency: 'ZAR',
      cancelUrl: `https://heartcart.shop/corporate-payment/${corporateOrderId}?status=cancelled`,
      successUrl: `https://heartcart.shop/corporate-payment/${corporateOrderId}?status=success`,
      failureUrl: `https://heartcart.shop/corporate-payment/${corporateOrderId}?status=failed`,
      metadata: {
        checkoutId: `corporate-${corporateOrderId}-${Date.now()}`,
        corporateOrderId: corporateOrderId.toString(),
        orderType: 'corporate',
        customerEmail: order.contactEmail,
        customerFullName: order.contactPerson,
        customerPhone: order.contactPhone || '',
        companyName: order.companyName
      },
      lineItems: order.items.map(item => ({
        displayName: `${item.productName} (${item.employeeName})`,
        quantity: item.quantity,
        pricingDetails: {
    ...
    console.log('📦 Creating corporate Yoco checkout session:', {
      corporateOrderId,
      amount: amountInCents,
      currency: 'ZAR',
    ...
    console.log('✅ Corporate Yoco checkout created successfully:', {
      checkoutId: checkoutResponse.id,
      redirectUrl: checkoutResponse.redirectUrl
    });
    ...
    console.error('❌ Failed to create corporate Yoco checkout:', error);
    res.status(500).json({ 
