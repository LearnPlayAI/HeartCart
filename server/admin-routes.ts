...
// Assign user to a sales rep
router.post("/sales-reps/:id/assign-user", isAdmin, asyncHandler(async (req: Request, res: Response) => {
  try {
    const repId = parseInt(req.params.id);
    ...
    // Assign user to the sales rep
    const updatedUser = await storage.assignUserToRep(userId, rep.repCode);
    
    if (!updatedUser) {
      return sendError(res, "User not found", 404);
    }

    logger.info("User assigned to sales rep by admin", { 
      userId,
      repId,
      repCode: rep.repCode,
      adminUserId: req.user?.id
    ...
    logger.error("Error assigning user to sales rep", { 
      error: error instanceof Error ? error.message : String(error),
      repId: req.params.id,
      userId: req.body.userId
    });
    return sendError(res, "Failed to assign user", 500);
  }
}));

// Remove user from a sales rep
router.post("/sales-reps/:id/remove-user", isAdmin, asyncHandler(async (req: Request, res: Response) => {
  try {
    const repId = parseInt(req.params.id);
    const { userId } = req.body;
    ...
    // Get the sales rep to verify it exists
    const rep = await storage.getSalesRepById(repId);
    if (!rep) {
      return sendError(res, "Sales rep not found", 404);
    }

    // Remove user from the sales rep
    const updatedUser = await storage.removeUserRepAssignment(userId);
    ...
    logger.info("User removed from sales rep by admin", { 
      userId,
      repId,
      repCode: rep.repCode,
      adminUserId: req.user?.id
    ...
    logger.error("Error removing user from sales rep", { 
      error: error instanceof Error ? error.message : String(error),
      repId: req.params.id,
      userId: req.body.userId
    });
