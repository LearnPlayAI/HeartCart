...
class ObjectStoreService {
  ...
  /**
   * Copy a file from one location to another in the object store
   * @param sourceKey The source object key
   * @param destinationKey The destination object key
   * @returns Promise resolving to the destination key if successful, null otherwise
   */
  async copyFile(sourceKey: string, destinationKey: string): Promise<string | null> {
    try {
      await this.ensureInitialized();
      
      // Get the file from the source location as a buffer
      const sourceFile = await this.getFileAsBuffer(sourceKey);
      ...
      // Determine content type
      const contentType = sourceFile.contentType || this.detectContentType(sourceKey);
      
      // Upload the buffer to the destination location
   ...
   * @param fileMappings Array of objects with sourceKey and destinationKey properties
   * @returns Promise resolving to array of successful destination keys
   */
  async copyFiles(fileMappings: Array<{sourceKey: string, destinationKey: string}>): Promise<string[]> {
    try {
      await this.ensureInitialized();
      
      const results = await Promise.all(
        fileMappings.map(async ({sourceKey, destinationKey}) => {
          return this.copyFile(sourceKey, destinationKey);
        })
      );
      
      // Filter out null results (failed copies)
      return results.filter((result): result is string => result !== null);
    } catch (error) {
      console.error('Error in bulk file copy operation:', error);
      return [];
    }
  }
}
